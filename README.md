...
- The committed cloud A/B run has **6 complete pairs / 12 successful calls**. Acoustic OFF/ON response P50 was **3,360.6/3,671.4 ms** and P95 **3,770.4/4,040.0 ms**. The paired mean delta was **+299.1 ms**; two repeats per scenario are descriptive, and this exceeds the proposed 30 ms production rollout gate. Acoustic inference itself was **0.76 ms P50** and signal-to-receiver transport **123.8 ms P50**. See `results/acoustic-livekit.json`.
...
**Current evidence:** all 12 acoustic cloud benchmark calls completed through LiveKit Cloud, Deepgram STT, OpenAI LLM and OpenAI TTS. A separate smoke replay received 94 expression predictions plus the normal answer. The dashboard, 46 automated tests, eight backchannel WAV fixtures, nine acoustic fixtures and 40 simulated pairs also work. The live demo is [livekit-backchannel.vercel.app](https://livekit-backchannel.vercel.app/); its token endpoint and Python voice worker are hosted separately from the static UI.
...
Preparation has a 650ms timeout. Exceptions are recorded and isolated from the normal response. Attempts, including failures, consume cooldown to prevent retry storms. Production uses four pre-generated listening clips: `mm-hmm`, `mmm`, `uh-huh`, and an occasional `go on`. They use `gpt-4o-mini-tts` with `alloy`, matching the default answer voice. No extra TTS call is placed on the hot path. Each session cycles through `mm-hmm, mmm, uh-huh, mm-hmm, go on, mmm`, without adjacent repeats. The simulator uses the same order and clip durations. Selection is recorded in `bc_clip_selected` events; the existing cooldown, cancellation and response priority rules apply to every clip. Variety is not semantic understanding: `go on` may still feel intrusive, and naturalness needs listening evaluation. Run `python scripts/generate_ack.py --clip mmm` (or `listening`, `uh-huh`, `go-on`) to regenerate an AI-generated clip offline. Adjacent JSON files record the prompt, voice and audio hash. Changing the answer voice may introduce a mismatch. Generated TTS is represented by the injectable async provider and slow/failing-provider tests; it is not a measured bonus comparison.
